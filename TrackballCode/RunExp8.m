...
function RunExp8(flyNum,flyExpNum,flyCond)
...
data.expNum = 8;                     % Alex's experiment number
...
speakerDirection = {'M'};           % 1 is left, 2 is right
data.stim.intensity = [0.05 0.1 0.2 0.4 0.8 1.6];
...
for n = 1:data.numTrials
    ...
    %% Trial information
        data.trialData.speaker = 'M';   % which speaker emits sound for a given trial
        data.trialData.intensity = data.stim.intensity(soundSource(n));
    ...
    % load stimulus
    ch0out = data.trialData.intensity.*data.stim.stimTrain;           %Middle speaker
    ch1out = zeros(length(data.stim.stimTrain),1);    
